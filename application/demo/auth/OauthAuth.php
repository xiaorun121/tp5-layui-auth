...
class OauthAuth extends OAuth
{
    ...
    {
        $user = new User();
        $info = $user::where('client_id',$client_id)->where('client_id is not null')->where('secret is not null')->field('client_id,secret,nickname')->find();

        $userList = [
            $info['client_id'] =>[
              'client_id' => $info['client_id'],//app_id
              'secret' => $info['secret'],
              'name' => $info['nickname']
            ]
        ];
